...
class BasePage():
    ...
    def my_find_element(self,locator,timeout=20,poll_frequency=0.5):
        ...
        try:
            ...
            return element
    def my_find_elements(self,locator,timeout=20,poll_frequency=0.5):
        '''
        元素定位
        :param locator:元素定位器，元组形式
        :param timeout: 超时时间，单位秒
        ...
        '''
        try:
            element = WebDriverWait(self.driver,timeout,poll_frequency).until(EC.presence_of_all_elements_located(locator))
        except:
            print(f'{locator}元素未找到')
            element = None
        finally:
